...
class SalesforceSubscriber
  ...
  def create_contact_push_topic
    ...
    PushTopic.create(sf_id: contact_topic, topic_type: 'contact') if contact_topic.present? && contact_topic.is_a?(String)
  ...
  def delete_account_topics
    PushTopic.account.each do |topic|
  ...
  def delete_contact_topics
    PushTopic.contact.each do |topic|
      @client.destroy('PushTopic', topic.sf_id)
